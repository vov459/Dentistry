...
class PatientReceptionEditView implements ViewInterface{
    ...
    public function render(): void
    {
        ...
        $addedAppeals = $rep->getAllAppeals($data->getId());
        $appeals = $appealsRep->findAll();
        ?>
        <form action="/db_app/save/patient_reception">
            <input type="hidden" name="id" value="<?=$data->getId()?>">
            <div class="input-group mb-3">
                <div class="input-group-prepend">
                    <span class="input-group-text bg-dark border-dark outline-dark text-white" id="basic-addon1">Дата</span>
                </div>
                <input required type="text" name="date" value="<?=$data->getDate()->format("Y-m-d")?>" class="border-dark form-control" aria-describedby="basic-addon1">
            </div>
            <div class="input-group mb-3">
                <div class="input-group-prepend">
                    <span class="input-group-text bg-dark border-dark outline-dark text-white" id="basic-addon1">Дата</span>
            ...
            <div class="input-group mb-3">
                ...
                <select class="border-dark custom-select" id="inputGroupSelect02" name="hour_reception_id">
                    ...
                    foreach ($hours as $item){
                        $selected = "";
                        if($item->getId() == $data->getHourReceptionId()){
                            $selected = "selected";
                        }
                        echo '<option '.$selected.' value="'.$item->getId() .'">'.$item->getHour()->format("H:i:s").
            ...
            <div class="input-group mb-3">
                ...
                <select class="border-dark custom-select" id="inputGroupSelect02" name="patient_id">
                    ...
                    foreach ($patients as $item){
                        $selected = "";
                        if($item->getId() == $data->getPatientId()){
                            $selected = "selected";
                        }
                        echo '<option '.$selected.' value="'.$item->getId() .'">'.
            ...
            <div class="medicines_wrap">
                <?php
                /* @var $addedAppeal Appeal */
                foreach($addedAppeals as $i=>$addedAppeal){
                    ?>
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">

                            <label class="input-group-text bg-dark text-white border-dark" for="inputGroupSelect02">Жалоба</label>
                        ...
                        <select class="custom-select medicineSelect border-dark" name="appeal_id[]">
                            <?php
                            /* @var $item Appeal */
                            foreach ($appeals as $item){
                                if($item->getId() == $addedAppeal->getId()){
